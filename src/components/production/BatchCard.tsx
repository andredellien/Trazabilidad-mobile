  ...
  return (
    ...
    >
      <View className="flex-row justify-between items-start mb-2">
        <Text className="text-lg font-bold text-gray-900">{batch.product_name}</Text>
        <View className={`px-2 py-1 rounded-full ${statusColors[batch.status].split(' ')[0]}`}>
          <Text className={`text-xs font-medium ${statusColors[batch.status].split(' ')[1]}`}>
      ...
      <View className="flex-row justify-between mt-2">
        <View>
          <Text className="text-gray-500 text-xs">Batch ID</Text>
          <Text className="font-medium">#{batch.batch_id}</Text>
        </View>
        <View>
          <Text className="text-gray-500 text-xs">Quantity</Text>
          <Text className="font-medium">{batch.quantity} units</Text>
        </View>
        <View>
